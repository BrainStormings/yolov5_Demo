...
class YOLOv5_GUI:
    ...
    def create_widgets(self):
        ...
        # 置信度阈值
        ttk.Label(threshold_frame, text="置信度阈值: {:.2f}".format(self.conf_thres.get())).pack(anchor=tk.W)
        conf_scale = ttk.Scale(threshold_frame, from_=0.01, to=1.0, variable=self.conf_thres,
                               command=lambda x: self.update_threshold_label('conf'))
        conf_scale.pack(fill=tk.X, pady=5)

        # IoU阈值
        ttk.Label(threshold_frame, text="IoU阈值: {:.2f}".format(self.iou_thres.get())).pack(anchor=tk.W)
        iou_scale = ttk.Scale(threshold_frame, from_=0.01, to=1.0, variable=self.iou_thres,
    ...
    def display_video_preview(self, video_path):
        ...
        try:
            ...
            messagebox.showerror("错误", f"无法预览视频: {str(e)}")
    ...
    def detect_video(self):
        ...
        def video_thread():
            try:
                ...
                self.root.after(0, lambda: messagebox.showerror("错误", f"视频检测失败: {str(e)}"))
                self.is_video_playing = False
