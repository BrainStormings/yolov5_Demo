    ...
    for scheduler_type, name in schedulers:
        ...
        if scheduler_type == "cosine":
            scheduler = create_lr_scheduler(
                optimizer, scheduler_type, 100, warmup_epochs=5, warmup_start_lr=1e-7, eta_min=1e-6
            )
        elif scheduler_type == "linear":
            scheduler = create_lr_scheduler(optimizer, scheduler_type, 100, warmup_epochs=5, warmup_start_lr=1e-7)
        elif scheduler_type == "step":
            scheduler = create_lr_scheduler(
                optimizer, scheduler_type, 100, warmup_epochs=5, warmup_start_lr=1e-7, step_size=30, gamma=0.1
            )
...
def main():
    ...
    # 测试3: 超参数进化
    _base_config, _evolved_config = test_hyperparameter_evolution()
